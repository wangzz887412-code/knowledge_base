...
- Canvas 精灵动画渲染，流畅的皮卡丘动画效果
- 智能行为检测：idle/sleep/skill 等多种动画状态自动切换
- 用户行为感知：鼠标/键盘操作重置计时，30秒无操作进入睡眠
- 与文件处理任务联动：上传/处理时自动切换桌宠状态
- 单击对话气泡、双击技能动画、拖拽移动
- 位置记忆（localStorage 持久化）

### 🔐 账户安全

- **忘记密码**: 登录页点击"忘记密码"，输入用户名获取临时密码
- **密码重置**: 使用临时密码登录后可在设置中修改密码
- **Token 验证**: 密码重置使用 UUID Token，支持过期验证
...
│   │   │   │   └── config.ts      # 桌宠配置
│   │   │   ├── DesktopPet.tsx      # 桌面宠物组件（旧版）
│   │   │   ├── FileDetailModal.tsx # 文件详情弹窗
│   │   │   └── ...
│   │   ├── contexts/
│   │   │   ├── TaskContext.tsx     # 任务上下文（桌宠联动）
...
- 🔐 新增忘记密码功能（临时密码 + Token 验证）
- 🐱 桌宠升级为皮卡丘 Canvas 精灵动画（DesktopPetV2）
- 📊 新增 TaskContext 任务上下文，桌宠与文件处理联动
- 🎨 智能行为检测：idle/sleep/skill 等多种动画状态自动切换
